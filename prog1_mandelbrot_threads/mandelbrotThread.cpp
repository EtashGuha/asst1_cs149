...
//
// Thread entrypoint.
void workerThreadStartBlocked(WorkerArgs * const args) {
    ...
    int startRow = args->threadId * rowsPerThread + (args->threadId < remainder ? args->threadId : remainder);
    int numRows = rowsPerThread + (args->threadId < remainder ? 1 : 0);
